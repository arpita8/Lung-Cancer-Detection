...
l = int(len(training)*0.2)
print(l)
#loop 10 times
for j in range(10):
	#initialize cnn each time
	x = tf.placeholder('float')
	y = tf.placeholder('float')
	np.random.shuffle(training)
	train_data = training[:-l]
	validation_data = training[-l:]
	test_pred = train_neural_network(x)
	file = 'test_pred'+str(j)
	np.savetxt(file, test_pred)
	for i in range(len(test_pred)):
		...
		patient = testing[i][1]
		col = 'cancer' + str(j)
		solution.ix[patient, col] = p

solution['mean'] = solution.mean(axis=1)
solution.to_csv("mean2.csv")